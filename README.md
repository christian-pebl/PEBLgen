...
> **⚡ New to this project?** See **[SETUP.md](SETUP.md)** for a super quick 3-step setup guide!
...
⚠️ **IMPORTANT:** The `supabase-client.js` file is committed to Git and contains your Supabase credentials (lines 6-7). If you make this repo public, your database would be exposed. Keep the repository private.
...
✅ **Same data on all machines** - Both laptops access the same database
✅ **Automatic sync** - Changes on one machine appear on the other (after refresh)
✅ **No manual backup needed** - Supabase handles backups automatically
✅ **File storage included** - All uploaded CSV files are in cloud storage
...
├── src/
│   ├── app/
│   │   ├── map-drawing/        # Main map application
│   │   ├── data-explorer/      # File management (deprecated)
│   │   └── ...
│   ├── components/
│   │   ├── map/                # Map components (Leaflet)
│   │   ├── pin-data/           # Data visualization components
│   │   └── ...
│   └── lib/
...
| **Data Explorer** | `/data-explorer` | ⚠️ Deprecated - use Map Drawing instead |
...
- ✅ Check `supabase-client.js` exists in project root
...
### "Data not showing up"
- ✅ Verify `supabase-client.js` has the correct credentials
- ✅ Check you're logged in with the same account
- ✅ Open browser console (F12) and look for "✅ [SUPABASE] User authenticated"
- ✅ Refresh the page (Ctrl+R or Cmd+R)

### "Port 8000 already in use"
- ✅ Stop other instances of the app
- ✅ Check if Python server is running (`python -m http.server 8000`)
...
⚠️ **Warning:** Changing the port will create a new browser storage context, so you may need to log in again.
